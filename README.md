...
- Hosting on Ubuntu using Gunicorn 19.10.0 without static content
  ...
  ```
 - Hosting on Docker using uWSGI web server on CentOS.
    ```
    ...
    $ docker build -t sample-django:$(git rev-parse --short HEAD) -f Dockerfile-python-centos .
    ...
    ```
  - Hosting on Docker using uWSGI web server on Ubuntu.
    ```
    # Building an Docker image with the tag which includes commit-id 
    # which will be used to maintaining multiple version for docker images.
    $ docker build -t sample-django:$(git rev-parse --short HEAD) -f Dockerfile-python-ubuntu .
